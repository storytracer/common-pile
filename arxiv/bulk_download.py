...
class BulkDownloader:
    ...
    def find_shard(self, article_id: str) -> Shard:
        ...
        if index == 0:
            logging.error(f"{article_id} appears too small for any shard")
            return None
        shard = self.shards[index - 1]
        if article_id > shard.end:
            logging.error(f"{article_id} appears too big for shard: {shard}")
        logging.debug(f"{article_id} should live in shard: {shard}")
        return shard
    ...
    def download_shard(self, shard: Shard):
        ...
        if os.path.exists(output_file) and not self.overwrite:
            logging.info(f"Downloading of {url} skipped as the file already exists.")
            return
        if self.dry_run:
        ...
        if md5 != shard.md5:
            logging.warning(f"md5 hash did not match for {shard.file_name}")
            return
        # Save the download compressed/unextracted (makes it easier to avoid
        # duplicate downloads.)
        logging.info(f"Saving shard to {output_file}")
        ...
        logging.info(f"Extracting shard at {output_file}")
