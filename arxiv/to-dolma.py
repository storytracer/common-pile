...
logging.basicConfig(
    level=logging.INFO,
    format="arxiv to-dolma: [%(asctime)s] %(levelname)s - %(message)s",
)
...
def interpolate_document(
    ...
    for m in re.finditer(r"(?P<cmd>\\input{(?P<filename>.*?)})", contents):
        ...
        try:
            ...
            logging.debug(f"Interpolating {m.group('filename')} into {article_id}")
...
def load_article_src(article_path: str, article_id: str) -> str:
    ...
    if not os.path.exists(article_path):
        logging.warning(f"Article `{article_id}` source is missing.")
        if not os.path.exists(f"{os.path.splitext(article_path)[0]}.pdf"):
            logging.warning(f"Article `{article_id}` pdf is missing too.")
        return []
    try:
        ...
        # If it isn't a tarfile, it is at least gzip compressed.
            ...
            logging.debug(f"Creating a document from single file for {article_id}")
